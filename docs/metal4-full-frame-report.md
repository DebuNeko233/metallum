...
| presentation    | yes         | **implemented in the frame encoder**: take the drawable, `waitForDrawable:` before the commit, the present triangle in the frame's own command buffer, `signalDrawable:` + present after it. **The presented drawable is readable on both generations** behind `-Dmetallum.drawableReadback=true` (the layer's `framebufferOnly` off, a copy into a shared buffer, read when the slot completes, one formatter in the shared layer) | yes - 30 presents a window; **and the two arms compared on one scene, one fixture, one switch, with the same settle (1366 and 1372 readbacks): both present the fixture's acceptance colour (pure green, red and blue at zero)**. Two pixel-level differences are measured and registered: the Metal 4 drawable's **alpha is 0 where the Metal 3 drawable's is 255** (invisible on an opaque layer, and the candidates are the two present passes' attachment load/store actions), and the Metal 3 image carries a **gentle radial ramp (230..255) where the Metal 4 one is flat** - neither claimed as a defect, both with an experiment | yes |
