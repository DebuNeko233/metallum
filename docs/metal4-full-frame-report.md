...
| presentation    | yes         | **implemented in the frame encoder**: take the drawable, `waitForDrawable:` before the commit, the present triangle in the frame's own command buffer, `signalDrawable:` + present after it. **And the presented drawable is now readable** behind `-Dmetallum.drawableReadback=true` (the layer's `framebufferOnly` off, a copy into a shared buffer after the present pass, read when the slot completes) | yes - 30 presents a window, and **150 readbacks of the presented drawable on a forced Metal 4 session: none black, 69 the loading screen's flat colour and 81 the world with structure in it**. Correctness (orientation, scaling, colour) is **not compared** - that needs the same switch on the Metal 3 arm, which is the next step | yes |
