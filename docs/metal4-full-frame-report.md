...
drawable written frame for frame, and the Metal 4 frame carries **alpha 0** where the Metal 3 frame carries the
alpha the shader wrote. So "does it look right" is now measurable in the pixels rather than blocked on the
display - and it is measured where it can be, not finished: the alpha's mechanism is not localised, orientation is
unproven (this fixture's picture is symmetric), and no other fixture's picture has been read. The counters, the
client's own chain lines, the readbacks and the absence of a fault are what the frame is known by.
...
shutdown, the real-pack ladder, the lifecycle gate - and the picture verdicts beyond the one fixture whose
pixels have been read. The remaining blockers above are the list; AUTO stays off this path on the intermittent
capability probe, and the migration's own success criterion cannot be claimed until the pictures that have been
read agree with the frames they are supposed to be.
