...
The two builds are paired by the integration contract rather than by a commit. Vitrail understands `API_VERSION` 1 and this repository answers `MetallumApi.apiVersion()`; a build that answers anything else is reported by Vitrail as incompatible rather than used. The head is named where a piece of evidence rests on one, because a run records the exact material it ran, but it is not how the pair is described: this repository has not been released yet, so there is no version on this side to name, and a commit hash on a branch that has since been merged names history rather than a head. `82a0c75e53e28390472b3c26b569cdc2335d90b4` is the newest code-bearing commit and is CI-verified: the Apple-Silicon guard, the Vitrail runtime and configuration contracts, the consolidated Metal and Vitrail contracts, `./gradlew build` and artifact capture all complete. Commits after it touch documentation, CI, the contract scripts and `.gitignore`, plus test comments, and change no backend behaviour.
