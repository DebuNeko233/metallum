...
Three conventions the contracts enforce rather than assume:
...
- **A workflow may not author a commit.** `ci.yml` is the only workflow that runs on a pull request,
  and a workflow that could commit or push anywhere in this repository is refused by shape rather
  than by name.
